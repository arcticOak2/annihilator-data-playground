...
class PrestoScriptGeneratorLegacyTest {
  ...
  void testGeneratePrestoScript_WithSingleUDF_ShouldGenerateCompleteScript() throws SQLException {
    ...
    assertTrue(
        script.contains(
            "hive -e \"CREATE TEMPORARY FUNCTION toUpperCase AS 'com.annihilator.dataphantom.udfs.TestUDF'\""));
    assertTrue(script.stream().anyMatch(line -> line.contains("echo \"Query:")));

    // Check that the query is in the script
    boolean queryFound =
        script.stream().anyMatch(line -> line.contains("SELECT toUpperCase('hello') as result"));
    assertTrue(queryFound, "Query should be present in the script");
  ...
  @Test
  void testGeneratePrestoScript_WithMultipleUDFs_ShouldGenerateCompleteScript()
      throws SQLException {
    // Given
    ...
    long downloadCommands =
        script.stream().filter(cmd -> cmd.startsWith("aws s3 cp") && cmd.contains("udfs/")).count();
    assertEquals(2, downloadCommands);

    long registerCommands =
        script.stream().filter(cmd -> cmd.contains("CREATE TEMPORARY FUNCTION")).count();
    assertEquals(2, registerCommands);

    // Check that the query is in the script
    boolean queryFound =
        script.stream()
            .anyMatch(line -> line.contains("SELECT toUpperCase('hello'), toLowerCase('WORLD')"));
    assertTrue(queryFound, "Query should be present in the script");
  ...
  void testGeneratePrestoScript_WithUDFNotFound_ShouldHandleGracefully() throws SQLException {
    ...
    assertTrue(
        script.contains(
            "echo \"Warning: UDF not found for ID: " + udfId + "\" | tee -a ${LOG_FILE}"));
