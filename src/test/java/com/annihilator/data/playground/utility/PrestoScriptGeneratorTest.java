...
/**
 * Comprehensive tests for UDFUtility script generation. These tests verify the actual generated
...
class PrestoScriptGeneratorTest {
  ...
  void testGeneratePrestoScript_WithoutUDFs_ShouldGenerateCleanScript() throws SQLException {
    ...
    assertTrue(
        script.stream()
            .anyMatch(line -> line.contains("LOG_S3_PATH=\"s3://test-bucket/data-phantom/logs/")));

    // Verify no UDF setup section
    assertFalse(
        script.stream().anyMatch(line -> line.contains("aws s3 cp s3://test-bucket/udfs/")));
    ...
    assertTrue(
        script.stream()
            .anyMatch(
                line ->
                    line.contains(
                        "presto-cli --catalog hive --schema core --output-format CSV_HEADER")));
    ...
    assertTrue(
        script.stream()
            .anyMatch(
                line ->
                    line.contains("aws s3 cp /tmp/output.csv s3://test-bucket/output/result.csv")));
  ...
  void testGeneratePrestoScript_WithUDFs_ShouldGenerateCompleteScript() throws SQLException {
    ...
    Task task = new Task();
    task.setUdfIds(
        "2a8e9ab9-3111-4892-8b55-9b13411bf4dc,3b9f0bc0-4222-5993-9c66-0ee44gff1fed"); // Two UDFs
    task.setQuery(
        "SELECT product_name, toUpperCase(product_name) as product_upper, toLowerCase(product_name) as product_lower, sum(quantity) as total_quantity FROM sales_data GROUP BY product_name");
    ...
    assertTrue(
        script.stream()
            .anyMatch(
                line ->
                    line.contains(
                        "aws s3 cp s3://test-bucket/udfs/string-utils.jar /tmp/string-utils.jar")));
    assertTrue(
        script.stream()
            .anyMatch(line -> line.contains("sudo cp /tmp/string-utils.jar /usr/lib/hive/lib/")));
    assertTrue(
        script.stream()
            .anyMatch(
                line ->
                    line.contains(
                        "CREATE TEMPORARY FUNCTION toUpperCase AS 'com.annihilator.dataphantom.udfs.StringUtils'")));
    assertTrue(
        script.stream()
            .anyMatch(
                line ->
                    line.contains(
                        "CREATE TEMPORARY FUNCTION toLowerCase AS 'com.annihilator.dataphantom.udfs.StringUtils'")));

    // Verify query execution
    assertTrue(script.stream().anyMatch(line -> line.contains("echo \"Query:")));
    assertTrue(
        script.stream()
            .anyMatch(line -> line.contains("hive -e \"SET hive.cli.print.header=true;")));
  ...
  void testGeneratePrestoScript_WithMissingUDF_ShouldHandleGracefully() throws SQLException {
    ...
    Task task = new Task();
    task.setUdfIds(
        "2a8e9ab9-3111-4892-8b55-9b13411bf4dc,nonexistent-udf-id"); // One valid, one missing
    task.setQuery(
        "SELECT campaign_name, toUpperCase(campaign_name) as campaign_upper FROM campaign_data GROUP BY campaign_name");
    ...
    assertTrue(
        script.stream()
            .anyMatch(line -> line.contains("CREATE TEMPORARY FUNCTION toUpperCase AS")));

    // Verify missing UDF warning
    assertTrue(
        script.stream()
            .anyMatch(line -> line.contains("Warning: UDF not found for ID: nonexistent-udf-id")));
