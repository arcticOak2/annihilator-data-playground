...
public class SQLQueryFieldExtractorTest {
    ...
    public void testComplexQueryWithCoalesceSum() {
        String complexQuery = "SELECT user_id, product_id, order_id, " +
        ...
        // First 10 fields: [user_id, product_id, order_id, total_qty, coalesce(sum(price), 0), coalesce(sum(discount), 0), coalesce(sum(tax), 0), product_category]
        ...
        assertTrue(fields.stream().anyMatch(field -> field.contains("coalesce")), 
    ...
    public void testConflicts_PrefixPreservation() {
        ...
        String query = "SELECT a.account_id, b.account_id, a.user_id, c.user_id FROM accounts a, users b, profiles c";
        List<String> fields = SQLQueryFieldExtractor.extractSelectedFields(query);
    ...
    public void testMixedScenario_SafeAndConflicting() {
        ...
        String query = "SELECT a.account_id, a.billing_timezone, b.account_id, c.user_name FROM accounts a, users b, profiles c";
        List<String> fields = SQLQueryFieldExtractor.extractSelectedFields(query);
        
        assertEquals(4, fields.size());
        assertTrue(fields.contains("a.account_id"), "Should keep prefix for conflicting account_id");
        assertTrue(fields.contains("billing_timezone"), "Should remove prefix from safe billing_timezone");
        assertTrue(fields.contains("b.account_id"), "Should keep prefix for conflicting account_id");
    ...
    public void testFunctionsWithPrefixes() {
        ...
        String query = "SELECT SUM(a.amount) AS total_amount, AVG(b.price) AS avg_price FROM orders a, products b";
        List<String> fields = SQLQueryFieldExtractor.extractSelectedFields(query);
    ...
    public void testComplexQueryWithMixedPrefixes() {
        ...
        String query = "SELECT a.account_id, a.billing_timezone, " +
        ...
        assertTrue(fields.contains("billing_timezone"), "Should remove prefix from billing_timezone (no conflict)");
    ...
    public void testSameTableMultipleReferences() {
        ...
        assertEquals(2, fields.size());
        assertTrue(fields.contains("account_id"), "Should remove prefix even with duplicate references");
    ...
    public void testNestedFunctionsWithPrefixes() {
        ...
        String query = "SELECT COALESCE(SUM(a.amount), 0) AS total, MAX(b.price) AS max_price FROM orders a, products b";
        List<String> fields = SQLQueryFieldExtractor.extractSelectedFields(query);
    ...
    public void testCaseStatementsWithPrefixes() {
        ...
        String query = "SELECT CASE WHEN a.status = 'active' THEN a.amount ELSE 0 END AS active_amount FROM accounts a";
        List<String> fields = SQLQueryFieldExtractor.extractSelectedFields(query);
    ...
    public void testWindowFunctionsWithPrefixes() {
        ...
        String query = "SELECT a.account_id, ROW_NUMBER() OVER (PARTITION BY a.status ORDER BY a.amount) AS row_num FROM accounts a";
        List<String> fields = SQLQueryFieldExtractor.extractSelectedFields(query);
    ...
    public void testSubqueriesWithPrefixes() {
        ...
        String query = "SELECT a.account_id, (SELECT COUNT(*) FROM orders o WHERE o.account_id = a.account_id) AS order_count FROM accounts a";
        List<String> fields = SQLQueryFieldExtractor.extractSelectedFields(query);
