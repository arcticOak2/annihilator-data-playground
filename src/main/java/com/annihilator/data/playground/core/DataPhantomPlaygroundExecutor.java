...
public class DataPhantomPlaygroundExecutor implements Runnable {
  ...
  public DataPhantomPlaygroundExecutor(
      ...
      PlaygroundExecutionType executionType,
  ...
  public void run() {
    ...
    try {
      ...
      if (temp.getCurrentStatus() == Status.RUNNING
          && (executionType == PlaygroundExecutionType.SCHEDULED
              || executionType == PlaygroundExecutionType.AD_HOC)) {

        logger.info(
            "Playground {} is already running. Skipping this execution.", playground.getName());
        return;
      }
      ...
      logger.info(
          "Playground {} execution completed. Success: {}, Failed: {}, Skipped: {}",
          playground.getName(),
          successCount,
          failureCount,
          skippedCount);
  ...
  private void updatePlaygroundMetaAfterCompletion() {
    try {
      ...
      logger.error(
          "Error updating playground completion and the run history for {}: {}",
          playground.getName(),
          e.getMessage(),
          e);
    }
  ...
  private void updatePlaygroundMeta(Queue<Task> rootTasks, UUID correlationId) {
    ...
    if (!rootTasks.isEmpty()) {
      logger.info(
          "Total {} root tasks found for playground: {}", rootTasks.size(), playground.getName());

      try {
        playgroundDAO.updatePlaygroundStart(
            playground.getId(), correlationId, System.currentTimeMillis(), Status.RUNNING);
      } catch (SQLException e) {
        logger.error(
            "Error updating playground start for {}: {}", playground.getName(), e.getMessage(), e);
      }
    }
  }

  private void updateTaskStatus(
      UUID correlationId, Task task, Status status, StepResult stepResult) {
    ...
    try {
      ...
      if (status == Status.RUNNING) {
        ...
        taskDAO.updateTaskCompletion(
            task.getId(), Status.IDLE, status, outputPath, logPath, correlationId);
      }
    } catch (SQLException e) {
  ...
  private void enqueueChildrenTasks(
      Task task,
      Queue<Task> queue,
    ...
    List<String> children =
        parentChildrenMap.getOrDefault(task.getId().toString(), new ArrayList<>());
  ...
  private boolean skipTask(
    ...
    if (limitedRun && !selectionMap.getOrDefault(task.getId().toString(), false)) {
      ...
      logger.info(
          "Skipping task {} (ID: {}) as it's not in the selection map for limited run.",
          task.getName(),
          task.getId());
      ...
      return true;
    } else if (Objects.nonNull(task.getLastCorrelationId())
        && task.getLastCorrelationId().equals(task.getCorrelationId())
  ...
  private void processTasks(DAGExecutionQueue dagExecutionQueue, UUID correlationId)
            ...
            future.thenApply(
                stepResult -> {
                  ...
                  if (isCancelled) {
                    ...
                    logger.info(
                        "Playground {} was cancelled. Not processing EMR completion for task {} (ID: {}).",
                        playground.getName(),
                  ...
                  if (stepResult.isSuccess()) {
                    ...
                    List<String> children =
                        parentChildrenMap.getOrDefault(
                            completedTask.getId().toString(), new ArrayList<>());
                    for (String child : children) {
                      logger.info("Enqueuing child task: {}", taskMap.get(child).getName());
                      queue.add(taskMap.get(child));
                    }
  ...
  private void updateChildrenTaskStatus(
      Task completedTask,
      Map<String, Task> taskMap,
      Map<String, List<String>> parentChildrenMap,
      UUID correlationId,
  ...
  public DAGExecutionQueue buildLevels(List<Task> tasks) {
    ...
    for (Task task : tasks) {
      ...
      if (task.getParentId() != null) {
        parentChildrenMap
            .computeIfAbsent(task.getParentId().toString(), k -> new ArrayList<>())
            .add(taskId);
      } else {
        rootTasks.add(task);
      }
  ...
  private static class DAGExecutionQueue {
    ...
    DAGExecutionQueue(
        Queue<Task> queue, Map<String, List<String>> parentChildrenMap, Map<String, Task> taskMap) {
