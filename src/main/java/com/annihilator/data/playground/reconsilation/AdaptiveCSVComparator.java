...
public class AdaptiveCSVComparator implements CSVComparator {
  ...
  private static final Logger LOGGER =
      org.slf4j.LoggerFactory.getLogger(AdaptiveCSVComparator.class);
  ...
  public void runReconciliation(String playgroundId) throws SQLException {
    Reconciliation reconciliation =
        reconciliationMappingDAO.findReconciliationMappingById(playgroundId);
  ...
  private void processSingleReconciliation(Reconciliation reconciliation) throws SQLException {
    ...
    LOGGER.info(
        "Reconciliation {} - Total file size: {} bytes, Using Bloom filter: {}",
        reconciliationId,
        totalFileSize,
        useBloomFilter);
  ...
  private void compareWithExactMatching(
      String reconciliationId, Task leftTask, Task rightTask, Map<String, String> columnsMapping)
      throws SQLException {
    ...
    s3Service.readFileLineByLine(
        leftTask.getOutputLocation(),
        line -> {
    ...
    try {
      String sampleCommonRowsS3Key =
          writeSampleDataToS3(sampleCommonRows, reconciliationId, "common");
      String sampleExclusiveLeftS3Key =
          writeSampleDataToS3(sampleExclusiveLeftRows, reconciliationId, "exclusive_left");
      String sampleExclusiveRightS3Key =
          writeSampleDataToS3(sampleExclusiveRightRows, reconciliationId, "exclusive_right");
      ...
      if (sampleCommonRowsS3Key != null) {
        ...
        String sampleCommonRowsS3Path =
            String.format("s3://%s/%s", bucketName, sampleCommonRowsS3Key);
        result.setSampleCommonRowsS3Path(sampleCommonRowsS3Path);
      }

      if (sampleExclusiveLeftS3Key != null) {
        String bucketName = s3Service.getBucketName();
        String sampleExclusiveLeftS3Path =
            String.format("s3://%s/%s", bucketName, sampleExclusiveLeftS3Key);
        result.setSampleExclusiveLeftRowsS3Path(sampleExclusiveLeftS3Path);
      }

      if (sampleExclusiveRightS3Key != null) {
        String bucketName = s3Service.getBucketName();
        String sampleExclusiveRightS3Path =
            String.format("s3://%s/%s", bucketName, sampleExclusiveRightS3Key);
        result.setSampleExclusiveRightRowsS3Path(sampleExclusiveRightS3Path);
      }

      LOGGER.info(
          "Sample data written to S3 - Common: {}, Left Only: {}, Right Only: {}",
          sampleCommonRowsS3Key != null
              ? "s3://" + s3Service.getBucketName() + "/" + sampleCommonRowsS3Key
              : "null",
          sampleExclusiveLeftS3Key != null
              ? "s3://" + s3Service.getBucketName() + "/" + sampleExclusiveLeftS3Key
              : "null",
          sampleExclusiveRightS3Key != null
              ? "s3://" + s3Service.getBucketName() + "/" + sampleExclusiveRightS3Key
              : "null");
    } catch (Exception e) {
    ...
    LOGGER.info(
        "Exact matching reconciliation completed - Left Total: {}, Right Total: {}, Common Unique: {}, Left Only Unique: {}, Right Only Unique: {}",
        totalLeftRows,
        totalRightRows,
        commonUniqueRows,
        leftExclusiveUniqueRows,
    ...
    try {
      reconciliationResultsDAO.upsertReconciliationResult(
          reconciliationId, result, "SUCCESS", "EXACT_MATCH");
      LOGGER.info(
          "Exact matching reconciliation result stored in database for ID: {}", reconciliationId);
    } catch (SQLException e) {
      LOGGER.error(
          "Failed to store exact matching reconciliation result in database for ID: {}",
          reconciliationId,
          e);
      storeFailedResult(reconciliationId);
      throw e;
  ...
  private void compareWithBloomFilter(
      String reconciliationId, Task leftTask, Task rightTask, Map<String, String> columnsMapping)
      throws SQLException {
    ...
    InMemoryBloomFilter<String> leftRowFilter =
        new InMemoryBloomFilter<>(
            reconciliationConfig.getEstimatedRows(), reconciliationConfig.getFalsePositiveRate());
    InMemoryBloomFilter<String> rightRowFilter =
        new InMemoryBloomFilter<>(
            reconciliationConfig.getEstimatedRows(), reconciliationConfig.getFalsePositiveRate());
    ...
    try {
      reconciliationResultsDAO.upsertReconciliationResult(
          reconciliationId, result, "SUCCESS", "PROBABILISTIC_MATCH");
      LOGGER.info(
          "Bloom filter reconciliation result stored in database for ID: {}", reconciliationId);
    } catch (SQLException e) {
      LOGGER.error(
          "Failed to store Bloom filter reconciliation result in database for ID: {}",
          reconciliationId,
          e);
      storeFailedResult(reconciliationId);
      throw e;
  ...
  private void buildBloomFilter(
      String s3Path,
      List<String> columns,
      InMemoryBloomFilter<String> bloomFilter,
      String fileType) {
  ...
  private CSVComparisonResult crossCompareWithBloomFilters(
      String leftPath,
      String rightPath,
    ...
    LOGGER.info(
        "Bloom filter bidirectional matching - Left→Right: {}, Right→Left: {}, Using minimum: {}",
        leftToRightMatches.get(),
        rightToLeftMatches.get(),
        commonRows);
    ...
    LOGGER.info(
        "Bloom filter reconciliation completed - Left Total: {}, Right Total: {}, Common: {}, Left Only: {}, Right Only: {} (99% accurate)",
  ...
  private void storeFailedResult(String reconciliationId) throws SQLException {
    try {
      ...
      reconciliationResultsDAO.upsertReconciliationResult(
          reconciliationId, failedResult, "FAILED", "ADAPTIVE_MATCH");
      LOGGER.info("Failed reconciliation result stored in database for ID: {}", reconciliationId);
    } catch (SQLException e) {
      LOGGER.error(
          "Failed to store failed reconciliation result in database for ID: {}",
          reconciliationId,
          e);
      throw e;
    }
  ...
  private Map<String, Integer> parseHeaderAndFindIndices(
      String headerLine, List<String> neededColumns) {
    Map<String, Integer> columnIndices = new HashMap<>();
  ...
  private String writeSampleDataToS3(
      List<String> sampleData, String reconciliationId, String type) {
    try {
      ...
      String fileName =
          String.format(
              "reconciliation_samples/%s/%s_%s.json",
      ...
      LOGGER.debug(
          "Sample data written to S3: {} (type: {}, rows: {})", s3Path, type, sampleData.size());
