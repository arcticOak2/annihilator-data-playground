...
public class DataPhantomResource {
    ...
    private void recover() {
        try {
            ...
            logger.info("Found " + runningPlaygrounds.size() + " playgrounds in RUNNING state for recovery.");

            for (Playground playground: runningPlaygrounds) {

                LimitedRunRequest limitedRunRequest = adhocLimitedInputDAO.getAdhocLimitedInputByRunId(playground.getCorrelationId().toString());

                if (limitedRunRequest != null) {
                    logger.info("Recovering limited adhoc run for playground {} with tasks: {}", playground.getName(), limitedRunRequest.getTasksToRun());
                }
    ...
    public Response reconciliationRun(@PathParam("reconciliation_id") String reconciliationId) {
        try {
            ...
            if (runningReconciliationSet.contains(reconciliationId)) {
                logger.info("Reconciliation {} is already running, skipping duplicate request", reconciliationId);
            ...
            java.util.concurrent.Future<?> future = adhocExecutorService.submit(() -> {
                try {
                    logger.info("Reconciliation run started for reconciliation ID {}", reconciliationId);
                    reconciliationManager.startIndividualReconciliation(reconciliationUUID);
                    logger.info("Reconciliation run completed for reconciliation ID {}", reconciliationId);
                } catch (Exception e) {
                    logger.error("Error during reconciliation run for ID {}: {}", reconciliationId, e.getMessage(), e);
                } finally {
            ...
            logger.error("Unexpected error in reconciliation run for ID {}: {}", reconciliationId, e.getMessage(), e);
            runningReconciliationSet.remove(reconciliationId);
            reconciliationFutures.remove(reconciliationId);
        ...
        try {
            ...
            if (playground == null) {
                ...
                return Response.status(Response.Status.NOT_FOUND)
                        .entity("Playground not found")
                        .build();
            }

            adhocExecutorService.submit(new DataPhantomPlaygroundExecutor(
                    playground,
                    taskDAO,
                    playgroundDAO,
                    historyDAO,
    ...
    public Response adhocLimitedRun(LimitedRunRequest limitedRunRequest) {
        try {
            ...
            logger.error("Error in limited adhoc run for playground {}", limitedRunRequest.getPlaygroundId(), e);
    ...
    public Response updatePlayground(Playground playground) {
        try {
            ...
            playgroundDAO.updatePlayground(playground.getId(), playground.getName(), playground.getCronExpression());
    ...
    public Response cancelPlaygroundRun(@PathParam("id") UUID id) {
        ...
        try {
            ...
            if (Objects.isNull(playground.getCurrentStatus()) || !playground.getCurrentStatus().equals(Status.RUNNING)) {
                return Response.status(Response.Status.BAD_REQUEST)
    ...
    public Response updateTaskQuery(Task task) {
        try {
            ...
            taskDAO.updateTaskQueryAndPlayground(task.getId(), task.getQuery(), task.getPlaygroundId(), task.getUdfIds());
    ...
    public Response getPlaygroundRunHistory(
        ...
        try {
            ...
            Map<String, Object> runHistory = historyDAO.getRunHistoryByPlaygroundId(playgroundUuid, limit);
            ...
            logger.error("Error fetching run history for playground {}: {}", playgroundId, e.getMessage(), e);
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
    ...
    public Response cancelReconciliationRun(@PathParam("reconciliation_id") String reconciliationId) {
        try {
            ...
            if (future != null && future.cancel(true)) {
                ...
                logger.warn("Failed to cancel reconciliation {} - future not found or already completed", reconciliationId);
                return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
    ...
    public Response getReconciliationMappingsByPlayground(@PathParam("playgroundId") String playgroundId) {
        try {
            List<Reconciliation> mappings = reconciliationMappingDAO.findReconciliationMappingByPlaygroundId(playgroundId);
    ...
    public Response getReconciliationMappingById(@PathParam("reconciliationId") String reconciliationId) {
        try {
            Reconciliation mapping = reconciliationMappingDAO.findReconciliationMappingById(reconciliationId);
    ...
    @PUT
    @Path("/reconciliation-mapping/{reconciliationId}")
    public Response updateReconciliationMapping(@PathParam("reconciliationId") String reconciliationId,
    ...
    @DELETE
    @Path("/reconciliation-mapping/{reconciliationId}")
    public Response deleteReconciliationMapping(@PathParam("reconciliationId") String reconciliationId) {
        try {
    ...
    public Response getReconciliationResult(@PathParam("reconciliationId") String reconciliationId) {
        try {
            ReconciliationResultResponse result = reconciliationResultsDAO.getReconciliationResult(reconciliationId);
        ...
        try {
            ...
            if (runningReconciliationSet.contains(reconciliationId)) {
                ...
                try {
                    ...
                    logger.error("Error fetching reconciliation result from database: {}", reconciliationId, e);
                    response.put("status", "FAILED");
