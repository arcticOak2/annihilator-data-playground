...
/**
 * Extracts selected fields from SQL queries, handling complex expressions, aliases, functions, CASE
 * statements, window functions, and subqueries.
 */
public class SQLQueryFieldExtractor {
  ...
  /**
   * Checks if it's safe to remove table prefix from a column name Returns true if removing the
   * prefix won't cause naming conflicts
   */
  private static boolean isSafeToRemovePrefix(
      String tableAlias, String columnName, List<String> allFields) {
  ...
  private static String extractFunctionAlias(String fieldStr) {
    ...
    if (openParen != -1) {
      ...
      if (closeParen != -1) {
        ...
        // For aggregate functions (SUM, AVG, COUNT, MIN, MAX), return the full function as field
        // name
