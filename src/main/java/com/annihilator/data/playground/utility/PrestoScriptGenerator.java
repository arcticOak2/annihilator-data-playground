...
/**
 * Script generator for Presto execution scripts with UDF support.
 * This class handles the dynamic generation of complete Presto execution scripts
 * including UDF setup (if any) and the main query execution.
 */
public class PrestoScriptGenerator {

    /**
     * Generates a complete Presto execution script for a given task.
     * This includes UDF setup (if any) and the main query execution.
     * 
     ...
     * @return Complete Presto execution script as List of strings
     * @throws SQLException if there's an error fetching UDF metadata
     */
    public static List<String> generatePrestoScript(Task task, UDFDAO udfDAO, String playgroundId, String queryId, String uniqueId, String bucket, String date, String tempFile, String outputPath) throws SQLException {
        return generatePrestoScript(task, udfDAO, playgroundId, queryId, uniqueId, bucket, "data-phantom", date, tempFile, outputPath);
    }
    
    public static List<String> generatePrestoScript(Task task, UDFDAO udfDAO, String playgroundId, String queryId, String uniqueId, String bucket, String pathPrefix, String date, String tempFile, String outputPath) throws SQLException {
        List<String> script = new ArrayList<>();
        
        // Generate script header
        script.add("#!/bin/bash");
        script.add("# Presto Query Execution Script");
        script.add("set -e");
        script.add("LOG_FILE=\"/tmp/presto-execution-" + uniqueId + ".log\"");
        script.add("LOG_S3_PATH=\"s3://" + bucket + "/" + pathPrefix + "/logs/" + date + "/presto-log-" + playgroundId + "-" + queryId + "-" + uniqueId + ".log\"");
        script.add("");
        ...
        if (task.getUdfIds() != null && !task.getUdfIds().trim().isEmpty()) {
            ...
            for (String udfId : udfIds) {
                ...
                script.add("hive -e \"CREATE TEMPORARY FUNCTION " + udf.getFunctionName() + " AS '" + udf.getClassName() + "'\"");
            }
        }
        ...
        if (task.getUdfIds() != null && !task.getUdfIds().trim().isEmpty()) {
            ...
            script.add("presto_exit_code=$?");
        } else {
            script.add("echo \"Query: " + task.getQuery() + "\" | tee -a ${LOG_FILE}");
            script.add("presto-cli --catalog hive --schema core --output-format CSV_HEADER --execute \"" + escapeForShell(task.getQuery()) + "\" > " + tempFile);
            script.add("presto_exit_code=$?");
        }
        
        script.add("if [ ! -f " + tempFile + " ] || [ ! -s " + tempFile + " ]; then");
        script.add("    presto_exit_code=1");
        script.add("fi");
        
        script.add("if [[ $presto_exit_code == 0 ]]; then");
