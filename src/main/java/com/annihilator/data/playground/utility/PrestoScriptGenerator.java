...
/**
 * Script generator for Presto execution scripts with UDF support. This class handles the dynamic
 * generation of complete Presto execution scripts including UDF setup (if any) and the main query
 * execution.
 */
public class PrestoScriptGenerator {

  /**
   * Generates a complete Presto execution script for a given task. This includes UDF setup (if any)
   ...
   */
  public static List<String> generatePrestoScript(
      Task task,
      UDFDAO udfDAO,
      String playgroundId,
      String queryId,
  ...
  public static List<String> generatePrestoScript(
    ...
    if (task.getUdfIds() != null && !task.getUdfIds().trim().isEmpty()) {
      ...
      script.add("echo \"Query: " + task.getQuery() + "\" | tee -a ${LOG_FILE}");
      script.add(
          "presto-cli --catalog hive --schema core --output-format CSV_HEADER --execute \""
              + escapeForShell(task.getQuery())
              + "\" > "
              + tempFile);
      script.add("presto_exit_code=$?");
