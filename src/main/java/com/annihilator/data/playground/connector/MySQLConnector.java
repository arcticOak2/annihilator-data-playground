    ...
    return CompletableFuture.supplyAsync(
        () -> {
          ...
          try {
            ...
            String fileName =
                String.format("sql-output-%s-%s-%s.csv", playgroundId, taskId, stepId);
            String localFilePath = new java.io.File(outputDir, fileName).getAbsolutePath();

            logger.info(
                "Executing SQL task {} for playground {} with step ID {}",
                taskId,
                playgroundId,
                stepId);
            ...
            try (Connection conn = mysqlDBConnection.getConnection()) {
              try (Statement stmt =
                ...
                try (ResultSet rs = stmt.executeQuery(query);
                  ...
                  logger.info(
                      "SQL query executed successfully, results written to {}", localFilePath);
                }
              }
            }

            String s3ObjectKey =
                String.format("%s/%s/%s/%s.csv", pathPrefix, currentDate, playgroundId, taskId);
            String uploadedS3Key = s3Service.uploadLocalFile(localFilePath, s3ObjectKey);
            String s3OutputPath =
                String.format("s3://%s/%s", s3Service.getBucketName(), uploadedS3Key);
            ...
            logger.info(
                "SQL task {} completed successfully, output uploaded to S3: {}",
                taskId,
                s3OutputPath);
            ...
            return new StepResult(
                stepId,
                StepState.COMPLETED,
                "SQL task executed successfully",
                s3OutputPath,
                null,
                taskId);
            ...
            return new StepResult(
                stepId, StepState.FAILED, "SQL task failed: " + e.getMessage(), null, null, taskId);
          }
        });
  }
