...
public class EMRServiceImpl implements EMRService {
  ...
  private final java.util.concurrent.ConcurrentHashMap<String, StepMetadata> stepMetadata =
      new java.util.concurrent.ConcurrentHashMap<>();

  public EMRServiceImpl(
      AWSEmrConfig awsEmrConfig,
  ...
  @Override
  public CompletableFuture<StepResult> submitTaskAndWait(
      String playgroundId, String queryId, String content, String taskType) {
    return CompletableFuture.supplyAsync(
        () -> {
          ...
          logger.info(
              "Starting task {} with {} retries", queryId, awsEmrConfig.getMaxStepRetries());

          for (int attempt = 0; attempt <= awsEmrConfig.getMaxStepRetries(); attempt++) {
            try {
              logger.info(
                  "Attempting task {} (attempt {}/{})",
                  queryId,
                  attempt + 1,
                  awsEmrConfig.getMaxStepRetries() + 1);
              ...
              StepResult result =
                  waitForStepCompletion(stepId, awsEmrConfig.getStepPollingInterval());
              lastResult = result;
              ...
              if (result.isSuccess()) {
                ...
                logger.warn(
                    "Task {} failed on attempt {}: {}", queryId, attempt + 1, lastFailureReason);
              ...
              logger.error(
                  "Task {} failed on attempt {} with exception: {}",
                  queryId,
                  attempt + 1,
                  e.getMessage(),
                  e);
              ...
              if (attempt < awsEmrConfig.getMaxStepRetries()) {
                logger.info(
                    "Retrying task {} after exception (attempt {}/{})",
                    queryId,
                    attempt + 2,
                    awsEmrConfig.getMaxStepRetries() + 1);
              }
          ...
          logger.error(
              "Task {} failed after {} attempts. Last failure: {}",
              queryId,
              awsEmrConfig.getMaxStepRetries() + 1,
              lastFailureReason);
  ...
  private StepResult waitForStepCompletion(String stepId, long pollIntervalMs)
    ...
    while (true) {
      ...
      if (status == StepState.COMPLETED) {
        ...
        return new StepResult(
            ...
            outputPath,
            logPath,
            metadata != null ? metadata.getQueryId() : null);
        ...
        String logPath = metadata != null ? metadata.getLogPath() : null;
        return new StepResult(
            stepId, status, reason, null, logPath, metadata != null ? metadata.getQueryId() : null);
      }
  ...
  private String submitTaskWithCustomOutput(
      Task task, String playgroundId, String queryId, String taskType) {
    ...
    try {
      ...
      String tempFile =
          String.format("/tmp/hive-output-%s-%s-%s.txt", uniqueId, playgroundId, queryId);

      StepConfig stepConfig;

      if (TaskType.PY_SPARK.name().equals(taskType)) {
        stepConfig =
            createSparkStepConfig(task.getQuery(), playgroundId, queryId, uniqueId, timestamp);
      } else if (TaskType.SPARK_SQL.name().equals(taskType)) {
        stepConfig =
            createSparkSQLStepConfig(
                task.getQuery(), playgroundId, queryId, uniqueId, timestamp, currentDate);
        outputPath =
            String.format(
                "s3://%s/%s/sparksql-output/%s/%s/%s/%s/",
                outputBucket, pathPrefix, currentDate, playgroundId, queryId, uniqueId);
      } else {
        stepConfig =
            createHivePrestoStepConfig(
                task,
                tempFile,
                outputPath,
                playgroundId,
      ...
      AddJobFlowStepsRequest request =
          AddJobFlowStepsRequest.builder().jobFlowId(currentClusterId).steps(stepConfig).build();
      ...
      stepMetadata.put(
          stepId,
          new StepMetadata(
              outputPath, logPath, playgroundId, queryId, uniqueId, currentDate, taskType));

      logger.info(
          "Task submitted successfully with step ID: {} and output path: {}", stepId, outputPath);
  ...
  private String generateSparkSQLScriptFromTemplate(
      String query,
      String playgroundId,
      String queryId,
      String uniqueId,
  ...
  private String getStepOutputPath(String stepId) {
    ...
    try {
      DescribeStepRequest request =
          DescribeStepRequest.builder().clusterId(currentClusterId).stepId(stepId).build();

      DescribeStepResponse response = emrClient.describeStep(request);
      String stepName = response.step().name();

      if (stepName != null
          && (stepName.startsWith("HiveQuery-")
              || stepName.startsWith("SparkQuery-")
              || stepName.startsWith("SparkSQLQuery-")
              || stepName.startsWith("PrestoQuery-"))) {
        String remaining;
        ...
        return String.format("s3://%s/%s/hive-query-%s.txt", outputBucket, pathPrefix, remaining);
      }
  ...
  private StepConfig createSparkStepConfig(
      String query, String folderName, String queryName, String uniqueId, String timestamp) {
    String pythonS3Key =
        s3Service.writeQueryToS3(
            query, String.format("spark-script-%s-%s-%s.py", folderName, queryName, uniqueId));
            ...
            HadoopJarStepConfig.builder()
                ...
                .args(
                    Arrays.asList(
                        ...
                        "cluster",
                        "--conf",
                        "spark.hadoop.hive.enforce.bucketing=true",
                        "--conf",
                        "spark.hadoop.hive.enforce.sorting=true",
                        pythonS3Path))
                .build())
        .build();
  }

  private StepConfig createSparkSQLStepConfig(
      String query,
      String folderName,
      String queryName,
      String uniqueId,
    ...
    String scriptContent =
        generateSparkSQLScriptFromTemplate(
            query, folderName, queryName, uniqueId, currentDate, awsEmrConfig.getS3Bucket());
    String pythonS3Key =
        s3Service.writeQueryToS3(
            scriptContent,
            String.format("sparksql-script-%s-%s-%s.py", folderName, queryName, uniqueId));
    String pythonS3Path = String.format("s3://%s/%s", awsEmrConfig.getS3Bucket(), pythonS3Key);
  ...
  private StepConfig createHivePrestoStepConfig(
      Task task,
      String tempFile,
      String outputPath,
      String folderName,
    ...
    if (TaskType.PRESTO.name().equals(taskType)) {
      List<String> scriptLines =
          PrestoScriptGenerator.generatePrestoScript(
              task,
              udfDAO,
              folderName,
              queryName,
    ...
    String scriptS3Key =
        s3Service.writeQueryToS3(
            scriptContent,
            String.format("%s-script-%s-%s-%s.sh", taskType, folderName, queryName, uniqueId));
            ...
            HadoopJarStepConfig.builder()
                ...
                .args(
                    Arrays.asList(
                        ...
                        "-c",
                        String.format(
                            "aws s3 cp %s /tmp/script.sh && chmod +x /tmp/script.sh && /tmp/script.sh",
                            scriptS3Path)))
                .build())
        .build();
  }
  ...
  private synchronized void ensureClusterReady() {
    try {
      ...
      logger.info(
          "Stored cluster ID {} is not alive, fetching any existing cluster ID", currentClusterId);
    ...
    try {
      ...
      if (clusterId != null) {
        ...
        if (state == ClusterState.WAITING || state == ClusterState.RUNNING) {
          ...
          logger.info(
              "Found bootstrapping cluster from CloudFormation: {} (state: {}), waiting for it to be ready",
              clusterId,
              state);
          ...
          logger.info(
              "Cluster {} from CloudFormation is in state: {}, will be recreated",
              clusterId,
              state);
        }
      }
  ...
  private String getClusterIdFromStackResources() {
    try {
      ListStackResourcesRequest request =
          ListStackResourcesRequest.builder().stackName(awsEmrConfig.getStackName()).build();
    ...
    try {
      ...
      if (stackClusterId != null) {
        for (ClusterSummary cluster : response.clusters()) {
          if (cluster.id().equals(stackClusterId)) {
            logger.info(
                "Found existing cluster from CloudFormation stack: {} (state: {})",
                cluster.id(),
      ...
      for (ClusterSummary cluster : response.clusters()) {
        if (cluster.name() != null && cluster.name().contains(awsEmrConfig.getClusterLogicalId())) {
          logger.info(
              "Found existing cluster with matching name: {} (state: {})",
              cluster.id(),
              cluster.status().state());
          return cluster.id();
        }
      ...
      logger.info(
          "Using first available existing cluster: {} (state: {})",
          firstCluster.id(),
          firstCluster.status().state());
  ...
  private void updateStackWithNewRandom() {
    try {
      ...
      if (e.getMessage().contains("No updates are to be performed")) {
        logger.warn(
            "Random parameter change didn't trigger resource updates - cluster won't be recreated");
        return;
      ...
      while (attempt < awsEmrConfig.getStackUpdateCheckMaxAttempt()) {
        try {
          DescribeStacksRequest request =
              DescribeStacksRequest.builder().stackName(awsEmrConfig.getStackName()).build();
