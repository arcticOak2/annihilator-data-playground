...
public class ReconciliationMappingDAOImpl implements ReconciliationMappingDAO {
  ...
  @Override
  public void createReconciliationMapping(
      String playgroundId, String leftTableId, String rightTableId, String map)
      throws SQLException {
    // Input validation
    ...
    String sql =
        "INSERT INTO reconciliation_mappings (reconciliation_id, playground_id, left_table_id, right_table_id, map, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)";
  ...
  @Override
  public List<Reconciliation> findReconciliationMappingByPlaygroundId(String playgroundId)
      throws SQLException {
    // Input validation
    if (playgroundId == null || playgroundId.trim().isEmpty()) {
      throw new IllegalArgumentException("Playground ID cannot be null or empty");
    ...
    String sql =
        "SELECT reconciliation_id, playground_id, left_table_id, right_table_id, map, created_at, updated_at FROM reconciliation_mappings WHERE playground_id = ?";
    ...
    try (Connection conn = metaDBConnection.getConnection();
      ...
      logger.info(
          "Found {} reconciliation mappings for playground: {}",
          allReconciliation.size(),
          playgroundId);
  ...
  public Reconciliation findReconciliationMappingById(String reconciliationId) throws SQLException {
    ...
    String sql =
        "SELECT reconciliation_id, playground_id, left_table_id, right_table_id, map, created_at, updated_at FROM reconciliation_mappings WHERE reconciliation_id = ?";
  ...
  public void updateReconciliationMapping(String reconciliationId, String map) throws SQLException {
    ...
    String sql =
        "UPDATE reconciliation_mappings SET map = ?, updated_at = ? WHERE reconciliation_id = ?";
