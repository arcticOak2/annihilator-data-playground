...
public class TaskDAOImpl implements TaskDAO {
    ...
    public List<Task> findTasksByPlaygroundRecursively(UUID playgroundId) throws SQLException {
        String sql = "SELECT id, name, playground_id, parent_id, created_at, modified_at, type, query, output_location, log_path, result_preview, task_status, last_run_status, correlation_id, last_correlation_id, udf_ids " +
                "FROM tasks " +
                "WHERE playground_id = ? " +
        ...
        try (Connection conn = metaDBConnection.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    task.setCorrelationId(rs.getString("correlation_id") != null ? UUID.fromString(rs.getString("correlation_id")) : null);
                    task.setLastCorrelationId(rs.getString("last_correlation_id") != null ? UUID.fromString(rs.getString("last_correlation_id")) : null);
                    ...
                    if (task.getParentId() != null) {
                        childrenMap.computeIfAbsent(task.getParentId(), k -> new ArrayList<>()).add(task.getId());
                    }
                }
            }
        }
    ...
    @Override
    public void updateTaskStatus(UUID taskId, UUID correlationId, Status newStatus) throws SQLException {
        String sql = "UPDATE tasks SET task_status = ?, correlation_id = ?, modified_at = ? WHERE id = ?";
        try (Connection conn = metaDBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public void createTaskAndUpdatePlayground(Task task) throws SQLException {
        ...
        try {
            ...
            String createTaskSql = "INSERT INTO tasks (id, name, playground_id, parent_id, created_at, modified_at, type, query, output_location, log_path, result_preview, task_status, last_run_status, correlation_id, last_correlation_id, udf_ids) " +
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    @Override
    public void updateTaskQueryAndPlayground(UUID taskId, String query, UUID playgroundId, String udfIds) throws SQLException {
        Connection conn = null;
        try {
            conn = metaDBConnection.getConnection();
            conn.setAutoCommit(false);

            String updateTaskSql = "UPDATE tasks SET query = ?, modified_at = ?, udf_ids = ? WHERE id = ?";
    ...
    @Override
    public void updateTaskCompletion(UUID taskId, Status currentStatus, Status lastRunStatus, String outputPath, String logPath, UUID correlationId) throws SQLException {

        String sql = Status.SKIPPED != lastRunStatus ?
