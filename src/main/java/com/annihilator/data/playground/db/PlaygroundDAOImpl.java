...
public class PlaygroundDAOImpl implements PlaygroundDAO {
  ...
  public void createPlayground(Playground playground) throws SQLException {
    ...
    String sql =
        "INSERT INTO playgrounds (id, name, created_at, modified_at, user_id, cron_expression) VALUES (?, ?, ?, ?, ?, ?)";
  ...
  public void updatePlayground(UUID id, String newName, String cron) throws SQLException {
    String sql =
        "UPDATE playgrounds SET name = ?, modified_at = ?, cron_expression = ? WHERE id = ?";
    try (Connection conn = metaDBConnection.getConnection();
        PreparedStatement ps = conn.prepareStatement(sql)) {
  ...
  @Override
  public void updatePlaygroundCompletion(
      UUID id, Status status, long endTime, int successCount, int failureCount, Status runStatus)
      throws SQLException {
  ...
  @Override
  public void updatePlaygroundStart(UUID id, UUID correlationId, long executedTime, Status status)
      throws SQLException {
    String sql =
        "UPDATE playgrounds SET "
            + "last_executed_at = ?, "
  ...
  public List<Playground> getAllPlaygroundsByStatus(Status status) throws SQLException {
    ...
    String sql =
        "SELECT id, name, created_at, user_id, modified_at, last_executed_at, cron_expression, correlation_id FROM playgrounds WHERE current_status = ?";
    try (Connection conn = metaDBConnection.getConnection();
        PreparedStatement ps = conn.prepareStatement(sql)) {
      ps.setString(1, status.name());
  ...
  public Map<String, Object> getPlaygrounds(String userId) throws SQLException {
    ...
    try (Connection conn = metaDBConnection.getConnection();
      ...
      while (rs.next()) {
        ...
        playground.setLastExecutedAt(rs.getLong("last_executed_at"));
        playground.setCronExpression(rs.getString("cron_expression"));
        playground.setCurrentStatus(
            rs.getString("current_status") != null
                ? Status.valueOf(rs.getString("current_status"))
                : null);
        playground.setLastRunStatus(
            rs.getString("last_run_status") != null
                ? Status.valueOf(rs.getString("last_run_status"))
                : null);
  ...
  public Playground getPlaygroundById(UUID id) throws SQLException {
    ...
    String sql =
        "SELECT id, name, created_at, user_id, modified_at, last_executed_at, cron_expression, correlation_id, current_status FROM playgrounds WHERE id = ?";
  ...
  public Map<String, Playground> getAllPlaygrounds() throws SQLException {
    ...
    String sql =
        "SELECT id, name, created_at, modified_at, user_id, cron_expression, last_executed_at FROM playgrounds WHERE cron_expression IS NOT NULL";
  ...
  @Override
  public Map<String, Playground> getPlaygroundsUpdatedOrCreatedAfter(long time)
      throws SQLException {

    String sql =
        "SELECT id, name, created_at, user_id, modified_at, last_executed_at, cron_expression FROM playgrounds WHERE modified_at > ? OR created_at > ?";
