...
public class ReconciliationResultsDAOImpl implements ReconciliationResultsDAO {
  ...
  @Override
  public void upsertReconciliationResult(
      String reconciliationId, CSVComparisonResult result, String status, String matchType)
      throws SQLException {
    ...
    try (Connection conn = metaDBConnection.getConnection()) {
      ...
      try (PreparedStatement insertPs = conn.prepareStatement(insertSql)) {
        ...
        logger.info(
            "Inserted reconciliation result for reconciliation ID: {} with status: {}",
            reconciliationId,
            status);
      }
  ...
  @Override
  public ReconciliationResultResponse getReconciliationResult(String reconciliationId)
      throws SQLException {
    if (reconciliationId == null || reconciliationId.trim().isEmpty()) {
      throw new IllegalArgumentException("Reconciliation ID cannot be null or empty");
    }
    ...
    String sql =
        "SELECT reconciliation_id, status, left_file_row_count, right_file_row_count, "
            + "common_row_count, left_file_exclusive_row_count, right_file_exclusive_row_count, "
            + "sample_common_rows_s3_path, sample_exclusive_left_rows_s3_path, "
    ...
    try (Connection conn = metaDBConnection.getConnection();
      ...
      try (ResultSet rs = ps.executeQuery()) {
        if (rs.next()) {
          ...
          response.setSampleExclusiveLeftRowsS3Path(
              rs.getString("sample_exclusive_left_rows_s3_path"));
          response.setSampleExclusiveRightRowsS3Path(
              rs.getString("sample_exclusive_right_rows_s3_path"));

          logger.info(
              "Retrieved reconciliation result for ID: {} with status: {}",
              reconciliationId,
              rs.getString("status"));
          return response;
        }
